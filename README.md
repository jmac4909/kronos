...
Use **Choose Project Discovery Folders** from the Work toolbar or Setup to select multiple parent folders in VS Code's native folder picker—for example, `IdeaProjects` and `PycharmProjects`. Kronos saves those machine-local roots and immediately opens **Discover and Register Local Projects**, where you choose which open workspace folders or discovered Git projects may be linked. Settings control root removal, scan depth, and result limit. Discovery is bounded and read-only, and nothing is registered until you select it. A ticket's **Project / Branch** action selects one primary local launch project while retaining its Jira/provider project associations. Kronos shows the project's current Git branch by reading `.git/HEAD` with Node built-ins; it never invokes Git or changes the repository. Future ticket-launched Claude terminals start in the linked folder. Existing terminals are never moved or sent a `cd` command.

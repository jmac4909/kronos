...
| Registered local project | `projectCatalog.ts` through `TerminalFirstState.ts` | Stable catalog key plus canonical real path and a separate optional nickname stored as `display_name`; shared ingress normalization for GitLab identity, credential-free Jenkins URL, SonarQube key, and conservative branch names | Duplicate canonical paths collapse to one identity; nickname set/clear never rewrites ticket/session links; Setup rejects malformed values; a Jira namespace never becomes a local project | Work, Projects, Sessions, Attention, project provider setup |

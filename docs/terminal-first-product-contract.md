...
Saving project integration data requests an immediate bounded provider poll in addition to the interval. Activation, Poll Now, Jira refresh, and relevant legacy Session changes may also request a coalesced poll. Provider polling configuration comes from the registered project's own GitLab, Jenkins, SonarQube, and branch-profile setup; no ticket link or terminal Session is required. When no MR is already known, project polling searches open MRs by the observed current source branch. Ticket-backed legacy fallback may then search by Jira key in title/description. Exactly one match is bound locally; ambiguous results are reported and never guessed. Explicit ticket-project links only project current provider facts onto that ticket and never create polling authority. Project MR and CI insertion controls work with an operator-owned project Session without adding a Jira context; all insertion controls fetch reviewed evidence only and are not provider-connect controls.

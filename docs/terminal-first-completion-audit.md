...
| --- | --- | --- |
| Product surface | Manifest gate enforces exactly Work, Sessions, Projects, Attention, 37 commands, and 10 settings. The activation harness registers and directly exercises every command. Runtime graph rejects cycles and dead runtime exports. | Pass |
...
- The activation harness now exercises all 37 contributed commands, including opening the Context Basket, adding another Jira context to a managed terminal, synthetic successful Jira refresh, editable MR/CI context insertion, project-routed provider context, and manual provider polling without using a live endpoint.
